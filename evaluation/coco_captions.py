...
class COCOCaptionsVanillaGenerationDataset(Dataset):
...
class COCOCaptionsVanillaGenerationCollator:
    ...
    def __call__(self, batch):
        ...
        batch_dict = {k: [item[k] for item in batch] for k in batch[0] if k not in ["gt_caption", "filename", "image_id"]}
...
class COCOCaptionsDualTowerGenerationDataset(Dataset):
    def __init__(self, dataset, tokenizer, image_processor, mp_image_token_length, total_samples):
...
class COCOCaptionsDualTowerGenerationCollator(VQADualCollator):
...
def compute_cider_score(model, cider_loader, device, tokenizer, max_new_tokens=30, max_samples=500, log_samples_path=None, is_dual_tower=False):
    model.eval()
