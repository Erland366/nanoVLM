...
class VQAIterableDataset(IterableDataset):
    """VQA Dataset for streaming/iterable datasets.

    This is a proper IterableDataset subclass that PyTorch DataLoader
    will recognize and handle correctly (no sampler needed).
    """

    def __init__(self, dataset, tokenizer, image_processor, mp_image_token_length,
                 relevance_min_rating=1, image_correspondence_min_rating=1,
                 visual_dependency_min_rating=1, formatting_min_rating=1):
        self.dataset = dataset
        self.tokenizer = tokenizer
        self.image_processor = image_processor
        self.mp_image_token_length = mp_image_token_length
