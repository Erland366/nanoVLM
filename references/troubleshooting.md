...
| Selective AC non-finite grads under compile | `grad_norm=nan` from first optimizer step when `compile=True` + selective AC | Selective policy saving generic matmul ops (`matmul_attention`) is unstable in this setup | Use attention-only selective policy (`attention_only`, now default); keep `matmul_attention` as backward-compatible alias to attention-only |
| Blackwell CUDA compatibility preflight | `RuntimeError: CUDA device compatibility check failed ... sm_120 ... supports [sm_50..sm_90]` (or older `no kernel image` errors without preflight) | PyTorch build lacks sm_120 support | Install a CUDA 12.8+ PyTorch build (e.g., torch 2.10.0+cu128) |
